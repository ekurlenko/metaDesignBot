...
def migrate_forward(op, old_orm, new_orm):
    op.create_table(new_orm.usermodel)
    op.create_table(new_orm.propertytypemodel)
    op.create_table(new_orm.roomtypemodel)
    op.create_table(new_orm.repairclassmodel)
    op.create_table(new_orm.ordermodel)
    op.run_data_migration()


def migrate_backward(op, old_orm, new_orm):
    op.run_data_migration()
    op.drop_table(old_orm.ordermodel)
    op.drop_table(old_orm.repairclassmodel)
    op.drop_table(old_orm.roomtypemodel)
    op.drop_table(old_orm.propertytypemodel)
    op.drop_table(old_orm.usermodel)
